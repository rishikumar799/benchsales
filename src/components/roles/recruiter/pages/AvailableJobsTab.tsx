...
import { recruiterStorage, RecruiterJob } from '../utils/recruiterStorage';
...
export default function AvailableJobsTab({ onNavigate }: AvailableJobsTabProps) {
  ...
  // Load from recruiterStorage on mount and listen to changes
  useEffect(() => {
  ...
  // Trigger request access action
  const handleRequestAccess = (jobId: string) => {
    const updatedJobs = jobs.map(j => {
      if (j.id === jobId) {
        return { ...j, accessStatus: 'pending' as const };
      }
      return j;
    });
    setJobs(updatedJobs);
    ...
    if (targetJob) {
      // Create a persistent request in candidate_access_requests
      const currentRequests = recruiterStorage.getAccessRequests();
      const newRequest = {
        id: `REQ-${Date.now().toString().slice(-5)}`,
        candidateId: 'pool',
        candidateName: 'Pool Access Request',
        jobId: targetJob.id,
        jobTitle: `${targetJob.title} at ${targetJob.company}`,
        requestDate: new Date().toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }),
        status: 'Pending' as const,
        notes: `Recruiter requesting workspace access for ${targetJob.title} at ${targetJob.company}.`
      };
      recruiterStorage.setAccessRequests([newRequest, ...currentRequests]);

      // Add a notification for requesting access
