...
interface MyAssignedJobsTabProps {
  onNavigate: (tab: string) => void;
}

export default function MyAssignedJobsTab({ onNavigate }: MyAssignedJobsTabProps) {
  ...
  // Map and compute only approved assigned jobs
  const jobs = useMemo(() => {
    return jobsRaw
  ...
  return (
    <div className="space-y-6 animate-fade-in text-left">
      ...
      {/* Header */}
      <div>
        <h1 className="text-3xl font-display font-bold text-app-text">My Assigned Jobs</h1>
        <p className="text-app-muted mt-1">These are open requirements specifically assigned to you or approved for your workspace by the BDM.</p>
      </div>
      ...
      <div className="p-4 rounded-2xl glass border border-app-border space-y-4">
        <div className="flex flex-col lg:flex-row gap-4 items-center">
          ...
          <div className="relative flex-1 w-full">
            ...
            <input 
              type="text" 
              placeholder="Search assigned jobs, companies..."
              value={searchQuery}
      ...
      {filteredJobs.length === 0 ? (
        <div className="p-12 text-center rounded-3xl bg-app-surface border border-app-border">
          ...
          <h3 className="font-display font-bold text-lg text-app-text">No Assigned Jobs Found</h3>
          <p className="text-app-muted text-sm mt-1">There are no approved or assigned requirements matching your filters.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredJobs.map((job) => (
            ...
            >
              ...
              <div className="flex justify-between items-start">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-[10px] font-mono font-bold uppercase px-2.5 py-0.5 rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 flex items-center gap-1">
                    <CheckSquare className="w-3 h-3" /> Assigned
                  </span>
                </div>
              </div>
              ...
              {/* Bottom Metrics Bar */}
              <div className="grid grid-cols-3 gap-2 p-3 rounded-2xl bg-app-bg/50 border border-app-border text-center text-[10px] font-mono font-bold">
                <div>
                  <div className="text-app-text">{job.submissionCount}</div>
                  <div className="text-app-muted uppercase text-[8px]">Submitted</div>
                </div>
                <div className="border-x border-app-border/60">
                  <div className="text-brand-blue">{job.shortlistCount}</div>
                  <div className="text-app-muted uppercase text-[8px]">Shortlist</div>
                </div>
                <div>
                  <div className="text-emerald-500">{job.hiredCount}</div>
                  <div className="text-app-muted uppercase text-[8px]">Hired</div>
                </div>
              </div>
              ...
              <div className="pt-2 flex items-center justify-between gap-4">
                ...
                <button 
                  ...
                  className="px-4 py-2 bg-brand-blue text-white hover:bg-opacity-95 text-xs font-extrabold rounded-xl transition-all shadow-md cursor-pointer"
                >
                  Submit Candidate
                </button>
              </div>
